...
def main():
    ...
    macro_file_name =   'phrases.json'
    try:
        with open(macro_file_name, 'r') as phrase_file:
            button_texts = json.load(phrase_file)
    except IOError:
        button_texts =  {   '-MACRO1-':'I would like a glass of water.',
                            '-MACRO2-':'Can we go to the store?',
                            '-MACRO3-':'I\'d like to drive the car.',
                            '-MACRO4-':'Who was that at the door?',
                            '-MACRO5-':'It\'s your turn to pick from the menu.',
                            '-MACRO6-':'Can you go get some cold medicine?',
                            '-MACRO7-':'Barney wants to walk and poop.',
                            '-MACRO8-':'This book is actually pretty good.',
...
if __name__ == '__main__':
    #Binary image of on / off states for our button, in text! Pulled from a demo. 
    #Kind of madness, tbh. TODO look into why one would do this.
...
##################### THE TRASHPILE - code I might need later or just to recall past failures #####################
# , vertical_alignment='center', justification='center'
#sg.Text('I would like a glass of water.', key='-MACRO1-', relief="raised", border_width=2, justification='c', s=macro_button_size)

    #window['-COL_LEFT-'].update(visible=False)
    #window[column_keys[0]].update(visible=False)
